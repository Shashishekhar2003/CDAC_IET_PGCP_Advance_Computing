    ...
    public static int BinarySearchNonRecursive(int[] arr,int searchnum) {
    	...
    	while(low<=high) {
    		...
    		if(arr[mid]==searchnum) {
    			System.out.println("number of comparisions: "+ cnt);
    			return mid;
    	...
    	System.out.println("Number of comparision: "+cnt);
    	return -1;
    
    }
    public static int binarySearchRecurssive (int [] arr,int searchnum,int low,int high) {
    	System.out.println("binary search callded low: " + "low: "+low+"high :"+high);
		if(low<=high) {
			int mid =(low+high)/2;
			if(arr[mid]==searchnum) {
				return mid;
